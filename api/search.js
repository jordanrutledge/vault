...
function parsePrice(t) { const m = (t || "").replace(/,/g, "").match(/[\d.]+/); return m ? parseFloat(m[0]) : 0; }

const BRANDS = ["Rolex","Patek Philippe","Audemars Piguet","Omega","Cartier","Hermès","Hermes","Chanel","Louis Vuitton","Gucci","Prada","Dior","Christian Dior","Goyard","Van Cleef & Arpels","Van Cleef","Tiffany","Bottega Veneta","Celine","Bulgari","Bvlgari","Nike","Christian Louboutin","IWC","Tudor","Breitling","Hublot","Panerai","Messika","Chopard","Fendi","Balenciaga","Saint Laurent","YSL","Valentino","Givenchy","Loewe","Miu Miu","Versace","Burberry","Chloe","Rimowa","Montblanc","David Yurman","John Hardy","TAG Heuer","Zenith","Jaeger-LeCoultre","Vacheron Constantin","A. Lange"];
...
function cleanName(name) {
  return name.replace(/Opens in a new window or tab\s*/gi, "").replace(/Sponsored\s*$/gi, "").replace(/\s+/g, " ").trim();
}
...
async function scrapeFashionphile(query, limit) {
  ...
  try {
    ...
    const r = await fetch(
      "https://www.fashionphile.com/search/suggest.json?q=" + encodeURIComponent(query) + "&resources[type]=product&resources[limit]=" + limit,
    ...
    for (const p of products) {
      ...
      results.push({
        ...
        condition: "Pre-owned", platform: "Fashionphile",
...
async function scrapeEbay(query, limit) {
  ...
  try {
    ...
    $("[data-view]").each((i, el) => {
      ...
      results.push({ name, brand: brand || query.split(" ")[0], price, condition: "", platform: "eBay (Sold)", url: href, imageUrl: img });
    });
...
function aggregate(listings) {
  ...
  for (const l of listings) {
    const normName = l.name.toLowerCase().replace(/opens in a new window.*/g, "").replace(/sponsored/g, "").replace(/[^a-z0-9]/g, " ").replace(/\s+/g, " ").trim();
    const modelMatch = normName.match(/\b(\d{5,6}[a-z]*)\b/);
    const key = modelMatch ? l.brand.toLowerCase() + "::" + modelMatch[1] : l.brand.toLowerCase() + "::" + normName.substring(0, 60);
    if (!groups[key]) groups[key] = { brand: l.brand, name: l.name, listings: [] };
    if (l.platform === "Fashionphile" || groups[key].name.includes("Opens in")) {
      groups[key].name = cleanName(l.name);
      groups[key].brand = l.brand;
    }
  ...
  for (const g of Object.values(groups)) {
    ...
    out.push({
      brand: g.brand, name: g.name, category: classify(g.name, g.brand),
      avgPrice: avg, lowPrice: Math.min(...cleanPrices), highPrice: Math.max(...cleanPrices),
      numListings: g.listings.length, sources: [...new Set(g.listings.map(l => l.platform))],
...
module.exports = async function handler(req, res) {
  ...
  let query, limit;
  if (req.method === "GET") { query = req.query.q; limit = parseInt(req.query.limit) || 15; }
  else { query = (req.body || {}).query; limit = (req.body || {}).limit || 15; }
  ...
  const items = aggregate(all);
  const queryWords = q.toLowerCase().split(/\s+/);
  const relevantItems = items.filter(item => {
    const itemText = (item.brand + " " + item.name).toLowerCase();
    return queryWords.some(w => itemText.includes(w));
  });

  const response = {
    query: q, totalListings: all.length,
    items: (relevantItems.length > 0 ? relevantItems : items).slice(0, 20),
    platforms: { ebay: { count: ebayResults.length }, fashionphile: { count: fpResults.length } },
